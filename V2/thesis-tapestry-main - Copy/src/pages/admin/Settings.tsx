...
const extractPhasesFromSemester = (sem: Semester) => {
  ...
  return [
    ...
    { start: s(sem.topic_registration_end).add(1, 'day'), end: s(sem.proposal_deadline) },
    { start: s(sem.proposal_deadline).add(1, 'day'), end: s(sem.thesis_deadline) },
...
const AdminSettings = () => {
  ...
  const updatePhase = (index: number, field: 'start' | 'end', date: dayjs.Dayjs | null) => {
    ...
    setPhaseDates((prev) => {
      ...
      // Khóa điểm giáp ranh (Chaining) để đồng bộ Backend. Nếu sửa Đầu của 1 Phase, thì Đuôi của Phase trước đó cũng phải chạy theo, và ngược lại. Tránh tình trạng hở Gaps bị mất data sau khi F5 vì Database chỉ có hàm ý 1 cột.
      if (field === 'end' && index < 4 && next[index + 1]) {
        next[index + 1] = { ...next[index + 1], start: date.add(1, 'day') };
      }
      if (field === 'start' && index > 0 && next[index - 1]) {
        next[index - 1] = { ...next[index - 1], end: date.subtract(1, 'day') };
      }
